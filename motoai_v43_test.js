...
(function(){
  ...
  function normalizeForCompare(s) {
    if (!s) return "";
    let t = s.toLowerCase();
    t = t.replace(/^(chào anh\/chị|dạ vâng|dạ|em chào anh\/chị)[.,\s]*/i, "");
  ...
  async function searchIndexHybrid(query, k = 3) {
    ...
    const exactModels = ["vision", "air blade", "wave", "sirius", "lead", "sh", "cub 50cc", "50cc", "xe điện", "xe ga", "xe số"];
    ...
    allChunks.forEach(d => {
      ...
      if (score > 0) {
        ...
        let entityBoost = 0;
        const lowText = d.text.toLowerCase();
        for (const m of exactModels) {
           if (lowQ.includes(m) && lowText.includes(m)) {
               entityBoost += 1.0; 
           }
        }
        score += entityBoost;
    ...
    allChunks.forEach(d => {
      if (qEmb) {
        if (d.semanticScore < 0.35 && d.bm25Norm === 0) {
          ...
          d.finalScore = Math.max((d.bm25Norm * 0.4) + (d.semanticScore * 0.6), d.bm25Norm * 0.85);
        }
      } else {
    ...
    return allChunks
      .filter(x => x.finalScore > threshold || x.bm25Norm >= 0.75)
  ...
  async function getRawAnswer(q, ctx, intents, newModel, newQty, newName, area, currentModel, lastBotState){
    if(lastBotState && lastBotState.state === "ASK_DURATION" && newQty && !newModel){
      const ans = composePrice(currentModel, newQty);
      return { ans, state: null, type: currentModel, qty: newQty };
    }
    ...
    if(topIntent === 'needPrice' || newQty || (currentModel && newQty)){
      if(newQty && !currentModel) {
        const ans = composePrice(null, newQty);
        return { ans, state: "ASK_MODEL", type: null, qty: newQty };
      }
      const qtyUse = newQty || (lastBotState && lastBotState.state === "ASK_MODEL" ? lastBotState.qty : null);
    ...
    if(currentModel && !newQty){
      ...
      return { ans, state: "ASK_DURATION", type: currentModel, qty: null };
    }

    const fallbackAns = polite(`Em chưa đủ dữ liệu để trả lời chắc chắn câu này. Anh/chị vui lòng gọi ${CFG.phone} để xác nhận thông tin.`);
    return { ans: fallbackAns, state: null };
  }
  ...
  async function deepAnswer(userText){
    ...
    for(let i=ctx.turns.length-1;i>=0;i--){
      if(ctx.turns[i].from === "bot"){
        lastBotState = ctx.turns[i];
        break;
      }
    }

    let currentModel = newModel;
    if(!currentModel && lastBotState && lastBotState.state === "ASK_DURATION"){
      currentModel = lastBotState.type || null;
    }
  ...
  async function sendUser(text){
    ...
    try {
      ...
      if(bots.length > 0) {
        const currentAnsNorm = normalizeForCompare(result.ans);
        let isRepeat = false;
        for(let b of bots) {
           const bNorm = normalizeForCompare(b.raw);
           if(bNorm === currentAnsNorm || (bNorm.length > 10 && jaccardSim(bNorm, currentAnsNorm) > 0.88)) {
              isRepeat = true;
              break;
           }
        }
      ...
      hideTyping();
      const err = `Có lỗi xử lý. Anh/chị vui lòng gọi ${CFG.phone} để được hỗ trợ.`;
      pushCtx({from:"bot", raw:err, state:null});
      addMsg("bot", err);
    } finally {
      sending = false;
    }
